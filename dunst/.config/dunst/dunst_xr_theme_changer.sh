...
xrdb_get () {
    output="$(xrdb -query | grep "dunst.$1" | awk 'NR==1{print $NF}')"
    default="$2"
...
declare -A theme_attr_dict=(
    ["urgency_low-background"]="\"$(xrdb_get 'low-background' '#2b303b')\""
    ["urgency_low-foreground"]="\"$(xrdb_get 'low-foreground' '#65737e')\""
    ["urgency_low-frame_color"]="\"$(xrdb_get 'low-frame' '#65737e')\""

    ["urgency_normal-background"]="\"$(xrdb_get 'normal-background' '#2b303b')\""
    ["urgency_normal-foreground"]="\"$(xrdb_get 'normal-foreground' '#a3be8c')\""
    ["urgency_normal-frame_color"]="\"$(xrdb_get 'normal-frame' '#a3be8c')\""

    ["urgency_critical-background"]="\"$(xrdb_get 'critical-background' '#2b303b')\""
    ["urgency_critical-foreground"]="\"$(xrdb_get 'critical-foreground' '#bf616a')\""
    ["urgency_critical-frame_color"]="\"$(xrdb_get 'critical-frame' '#bf616a')\""
)
